...
class buyQueue {
    buyHist: buyQueueInt = {}
    amountLim = 0.0001
    tokenCounter: {[token: string]: number} = {}
    ...
    addBuy(tokenCA: string, dateStr: string, amount: number, unitPrice: number|undefined): void {   
     ...
     calcGainsSell(tokenCA: string, sellDateStr: string, amountSell: number, priceSell: number|undefined): sellResult {
         ...
         while (amountLeftToSell > this.amountLim) {
            if (this.buyHist[tokenCA].length === 0) {throw `Now more tokens left to sell for tokenCA ${tokenCA} with ${amountLeftToSell} left to sell, on ${sellDateStr}.`}
